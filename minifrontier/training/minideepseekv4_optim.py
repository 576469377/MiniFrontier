...
def deepseek_parameter_specs(model):
    specs = {}
    ...
    for name, p in model.named_parameters():
        if name.startswith(("vision.", "image_")):
...
class MiniDeepSeekV4Optimizer(SemanticOptimizer):
    def __init__(self, model, *, lr, adam_lr=None, weight_decay=0.1, eps=1e-20):
        super().__init__(
            model,
            deepseek_parameter_specs(model),
            lr=lr,
