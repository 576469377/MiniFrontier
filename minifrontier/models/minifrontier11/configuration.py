"""MF1.1-owned configuration; field order retains the original 1.1 checkpoint schema."""
...
@dataclass
class MiniFrontier11Config:
    model_version: str = "1.1-reference-v1"
    ...
    vision_config: MF11VisionConfig = field(default_factory=MF11VisionConfig)

    def __post_init__(self):
        if isinstance(self.vision_config, dict):
            self.vision_config = MF11VisionConfig(**self.vision_config)
        ...
        if self.model_version != MF11_VERSION:
            raise ValueError("unknown MF1.1 model version")
        if self.mtp_enabled or self.mtp_loss_coef != 0:
    ...
    @classmethod
    def tiny(cls, vocab_size=320, *, model_version=MF11_VERSION):
        return cls(
            ...
            vision_config=MF11VisionConfig(
                depth=1,
                hidden_size=24,
                num_heads=2,
