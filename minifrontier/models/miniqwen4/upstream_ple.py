...
# Adaptation: extracted dependency-light PLE classes from Transformers commit
# 4177486a9f199bd7be520eff14431071d5d41ec5. Computation and cache protocol unchanged.
# Only external type names are aliases; no full Transformers import is required.
# ruff: noqa: SIM110 -- Preserve the upstream prime-search control flow.
...
def _build_layer_multipliers(
    unigram_vocab_size, ngram_size, ple_layer_index, seed: int
) -> torch.Tensor:
    max_long = (1 << 63) - 1
...
class Qwen4ExpTextNGramEmbedding(nn.Module):
    def __init__(
        ...
        self.eos_token_id = (
            config.eos_token_id[0] if isinstance(config.eos_token_id, list) else config.eos_token_id
        )
        ...
        padded_vocab_size = (
            math.ceil(self.total_vocab_size / ngram_vocab_divisor) * ngram_vocab_divisor
        )
    ...
    def forward(self, input_ids: torch.Tensor, past_key_values: Cache | None) -> torch.Tensor:
        ...
        execution_device = (
            ...
            if self.ngram_embedding.weight.device.type != "meta"
            else None
        )
...
class Qwen4ExpTextPLELayer(nn.Module):
    ...
    def _short_conv(
        self, hidden_states: torch.Tensor, past_key_values: Cache | None
    ) -> torch.Tensor:
