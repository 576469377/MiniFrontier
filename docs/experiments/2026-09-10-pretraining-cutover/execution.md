...
有限样本上的 0.5% 哈希桶未让每个来源的实际验证组比例都达到 0.5%。已按固定次序选出 75 个额外验证组，涉及 75 条记录、121,956 个参考 token，并排除了现有抽查样本组；这份补充划分尚未应用到消费者。接下来应让 tokenizer 与编码共用修正后的分区，不能只提高审计报告中的数字。最终词表、质量复核和模型数据验收仍未完成。
