...
两组学习实验均已实际启动，并从第 2 步检查点恢复到新的 CUDA 进程继续更新。冻结训练源码为 `f2e96ca22fa5122d6520d932a1e70e2eee00fef6`；[学习快照](learning-snapshot.json)记录采集时间、当前更新、初次验证和恢复点哈希，[3e-4 曲线](adamw-lr3e-4.metrics.jsonl)与 [1.5e-4 曲线](adamw-lr1.5e-4.metrics.jsonl)保留原始数值。这是运行中的早期快照，不能据此前几步损失判定哪组更好。恢复检查确认实际加载及计数连续，未在这次 GPU 实验中另跑不间断轨迹做逐位比较。
