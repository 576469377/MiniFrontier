...
本页适用于三个来源模型；MF1 使用[独立媒体 Demo](minifrontier1.md#导出和诊断-demo)。下面以物理 GPU 0 为例。`--root` 指向你保存训练结果的目录；需要先有本地检查点。
...
如需只观察某几组试验，可追加 `--experiment-root <相对目录>`，此参数可重复传入。例如检查点位于 `outputs/my-experiment/` 时使用 `--experiment-root my-experiment`。页面将选中的试验与历史试验分别显示；不传此参数则展示全部实验检查点。
...
启动前通过 `nvidia-smi` 确认所选设备及显存可用。Demo 单卡运行。
