...
删除上一轮执行升级的 4 份旧恢复副本，以及诊断目录中未被引用的 11 份 `.refresh` 残留，实际回收 **18.20 GiB**。其中 7 份残留只是硬链接别名，未计入释放空间；对应原文件仍在。

六条正式训练的当前检查点、K1/Q1/D1/P0 阶段终点、7 份本轮执行升级回退副本及延期 instructions 检查点保留。诊断控制器仍在使用的 13 份推理快照也保留。删除前逐项复核进程引用与文件身份，删除后确认六条训练由原进程继续更新。

旧执行升级的四个早期恢复点不再支持精确续训，后续完整检查点仍可使用。日志、指标、数据、tokenizer 和源码副本保持完整。逐文件结果与空间口径见[清理清单](2026-09-14-checkpoint-cleanup.json)。
