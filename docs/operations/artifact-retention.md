...
清理 5 份旧来源模型恢复检查点、1 个废弃保存临时文件，以及 5 份已结束的 MF1 实验恢复检查点。MF1 先导出 float32 推理权重，每份 2,305 个张量与原检查点、CPU 加载结果一致；配置、tokenizer、step 和校验值一并保留在各运行的 `retained-inference/`。

共删除 **11 个文件**，扣除新增推理副本后净回收 **36.28 GiB**。当前四条正式训练、K1/D1/P0 阶段父检查点、Qwen 的恢复依赖及两份延期 instructions 恢复状态继续保留。历史 instructions 恢复读取自身完整检查点；其原 SFT 初始化文件退役后的限制见[逐文件清单](2026-09-13-checkpoint-cleanup.json)。原始日志、评估、数据和源码副本保留。
