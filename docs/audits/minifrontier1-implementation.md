...
依据 [2026-09-09 融合方案](../training-strategies/2026-09-09/04-MiniFrontier1.0-原生多模态融合架构与全流程实现方案.md)新增独立模型和入口；原方案正文保持不变。此页记录实现范围，不把 M1–M3 的工程测试当作 M4–M8 的训练能力结果。
...
现有三模型继续使用冻结来源，不停止或迁移其训练。MF1 的小规模过拟合另用 `outputs/mf1-source-reference-v2` 冻结副本，CPU 执行；默认预留 50 GiB 磁盘。检查点与原始诊断媒体留在 Git 忽略的 `outputs/`；公开轻量报告不包含模型权重和大数据。
