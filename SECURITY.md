...
Report vulnerabilities privately to the repository maintainers using the hosting platform's private reporting channel when available. Do not include secrets or exploit payloads in public issues.
...
The browser demo binds to 127.0.0.1 by default and accepts only locally discovered educational checkpoints. It is intended for local use or SSH forwarding; it has no public-service authentication. Checkpoints are loaded with torch.load(weights_only=True), and generated text is rendered as text content. The demo does not execute generated code.
