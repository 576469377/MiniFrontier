...
MiniFrontier 提供小规模语言与多模态模型的结构实现、数据处理、训练、恢复、评估和推理工具。目前包含 **MF1.0 / MF1.1、MiniDeepSeek-V4 / V4.1、MiniQwen4、MiniKimi-K3 六个模型版本**。MF1 系列组合递推注意力、历史压缩、稀疏检索和潜在空间专家；来源模型分别保留其结构，供源码对照和实验。

> [!NOTE]
> 当前为 **v0.1.0 研究预览**。六个版本均已启动正式预训练，完整训练与能力评估尚未完成，仓库不附带聊天权重。新增 MF1.1 与 V4.1 独立训练，保留旧版本作对照；结构变化不代表效果已经提升。训练安排见[预训练主计划](docs/pretraining-plan.md)。

## 模型家族

各版本从随机初始化开始训练。点击模型名称查看结构图、模块计算、配置与实现代码。

| 模型 | 参数量（研究配置） | 输入接口 | 主要结构 |
|:---|---:|:---|:---|
| **[MiniFrontier1.1](docs/models/minifrontier11.md)** | **211M** | 文本、图像、视频帧 | KDA / CSA / QSA-MLA、Single-Pass mHC、LatentMoE、lookup；主干无 MTP |
| [MiniDeepSeek-V4.1](docs/models/minideepseekv41.md) | 242M | 文本；视觉流程待接通 | CED / CSA2、Single-Pass mHC、Engram；直接稀疏预训练 |
...
参数量按各自研究配置统计，输入接口与训练覆盖分开说明。MF1 系列结构图依据本地实现绘制；来源模型说明上游资料、缩减范围与实现差异，已有报告图见[出处索引](docs/assets/upstream/README.md)。
...
| 模型 | 精确浮点参数量 | 统计范围 |
|:---|---:|:---|
| MiniFrontier1.1 | 210,859,393 | 主干与视觉模块，无 MTP |
| MiniDeepSeek-V4.1 | 242,393,544 | 文本主干，无 MTP |
...
表中为各研究配置的浮点参数总数，MoE 的总参数量与单次前向激活参数量不同。MiniDeepSeek-V4 另有 262,144 个固定整数 hash 路由项，不计入上述浮点参数；直接对全部 `model.parameters()` 求和会得到 244,245,616。微型示例使用更小的配置。

MF1 系列的组合方案由本项目设计；MiniQwen4 的名称来自所参考源码中的 `qwen4_exp` 模块。各模型的来源版本与修改对应关系见[第三方说明](THIRD_PARTY_NOTICES.md)和[MF1 来源映射](configs/minifrontier1/source-map.json)。

</details>

2026-09-14 新增的 MiniDeepSeek-V4.1 与 MF1.1 分别采用 2.5B、3B 主 CE token 预算。结构取舍、训练阶段及与旧版的比较条件见[联合方案](docs/training-strategies/2026-09-14/07-v41-mf11-implementation-and-training.md)。

## MF1 架构

MF1.0 与 MF1.1 均采用 **16 层、hidden 512、32K 词表**。文字与 ViT 视觉特征进入同一序列：第 4、12 层用 CSA 压缩历史，第 8、16 层用 QSA-MLA 读取历史，其余层用 KDA 递推。各层包含 LatentMoE 与共享 FFN，浅层 lookup 补充局部特征。

| 版本差异 | MF1.0 | MF1.1 |
|:---|:---|:---|
| 四路残差混合 | GR | Single-Pass mHC |
| 正式训练优化器 | AdamW | 矩阵 Muon / Sinkhorn，标量等使用 AdamW |
| 主干辅助预测 | MTP | 关闭 MTP；独立草稿模型待实现 |
| 注意力课程 | dense → indexer → sparse | 沿用相同课程 |

<p align="center">
  <a href="docs/assets/minifrontier11-architecture.svg"><img src="docs/assets/minifrontier11-architecture.svg" alt="MF1.1 结构：文字和视觉特征汇入四路残差状态，经 KDA、CSA、QSA-MLA 与 LatentMoE 处理；Single-Pass mHC 连接子层，主干不含 MTP。" width="960" /></a>
</p>

[MF1.1 模型说明](docs/models/minifrontier11.md) · [下载结构图](docs/assets/minifrontier11-architecture.svg) · [训练入口与版本选择](docs/guides/minifrontier1.md)

<details>
<summary><b>MF1.0 结构与已有实验</b></summary>

<p align="center">
  <a href="docs/assets/minifrontier1-detail.svg"><img src="docs/assets/minifrontier1-detail.svg" alt="MF1.0 的 228M 结构：16 层注意力顺序、四路 GR 读写、LatentMoE，以及主输出和 MTP 分支。" width="960" /></a>
</p>

[模型说明](docs/models/minifrontier1.md) · [完整结构图](docs/assets/minifrontier1-detail.svg) · [小配置学习实验](docs/experiments/mf1-reference-v2/README.md)
...
以下示例在 CPU 上生成数据、训练并加载 **MF1.0** 微型权重。需要 **Linux、Python 3.11+** 和 **[uv](https://docs.astral.sh/uv/getting-started/installation/)**；Windows 可使用 WSL2。安装依赖后无需下载语料或模型权重。
...
CUDA_VISIBLE_DEVICES='' OMP_NUM_THREADS=2 MINIFRONTIER_MIN_FREE_GIB=1 \
  uv run minifrontier mf1 quickstart --model-version 1.0 \
    --device cpu --output outputs/mf1-quickstart
```
...
每次重跑需换一个新输出目录。微型示例用于检查流程，生成结果可能为空或错误；耗时和显存不适用于研究配置。MF1.1 共用 `mf1` 命令，通过 `--model-version 1.1` 选择版本，配置与限制见[模型页](docs/models/minifrontier11.md)。
...
| 任务 | 入口 |
|:---|:---|
| 用自己的数据训练 MF1.0 / MF1.1 | [数据格式与版本选择](docs/guides/minifrontier1.md) |
| 查看 MiniDeepSeek-V4.1 的配置与训练入口 | [V4.1 模型页](docs/models/minideepseekv41.md) |
| 在 CPU / 单张 3090 上运行 Kimi、Qwen、DeepSeek-V4 微型示例 | [最小示例与实测耗时](docs/guides/quickstart.md) |
...
<details>
<summary><b>MF1.0 小配置：600 次更新的学习结果</b></summary>
